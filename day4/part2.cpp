...
static std::tuple<Section const &, Section const &> find_smallest_section(PairSection const & section_pair)
{
  auto const & smallest_section = section_pair[0];
  auto const & other_section = section_pair[1];
  if (other_section.begin_ < smallest_section.begin_) {
    return { other_section, smallest_section };
  }
  return { smallest_section, other_section };
}

static bool is_section_overlapping(PairSection const & pair_section)
{
  const auto & [smallest_section, other_section] = find_smallest_section(pair_section);
  const auto smallest_section_end = smallest_section.begin_ + smallest_section.size_;

  return smallest_section.begin_ <= other_section.begin_ &&
          other_section.begin_ <= smallest_section_end;
